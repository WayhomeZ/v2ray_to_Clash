...
* **过滤模块设计**（已全部集成于 `scripts/purify.py`）：
  * **格式规范过滤模块（已启用）**
    * **子模块 1：SS Cipher 过滤子模块**
      * **解决的问题**：Shadowsocks 节点中 `cipher` 字段被错误填充为 `ss`，以及使用已弃用的 `chacha20-poly1305` 加密。
      * **解决方法**：直接丢弃 `cipher` 为 `ss` 的节点，并将 `chacha20-poly1305` 自动更正为 `chacha20-ietf-poly1305`。
      * **预计结果**：输出的 Shadowsocks 节点 100% 兼容 Mihomo 规范，记录过滤数量与耗时。
    * **子模块 2：Reality Short-ID 过滤子模块**
      * **解决的问题**：非偶数长度、非十六进制或长度超限（>16字节）的 REALITY `short-id` 导致解析崩溃。
      * **解决方法**：检查十六进制合法性、偶数长度以及长度上限限制，剔除不合规节点。
      * **预计结果**：保证所有 REALITY 节点 short-id 语法合规，记录过滤数量与耗时。
    * **子模块 3：UUID 格式过滤子模块**
